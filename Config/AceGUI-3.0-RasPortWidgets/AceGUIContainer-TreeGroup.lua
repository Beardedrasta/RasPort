...
local Type, Version = "TreeGroup-RasPort", 48 -- 45 by OA -- 46 by DF -- 48 backdrop
-- e
...
local function UpdateButton(button, treeline, selected, canExpand, isExpanded)
	...
	if ( level == 1 ) then
		button:SetNormalFontObject("GameFontNormal")
		button:SetHighlightFontObject("GameFontHighlight")
		button.text:SetPoint("LEFT", (icon and DEFAULT_ICON_SIZE + ((DEFAULT_TAB_HEIGHT - DEFAULT_ICON_SIZE)/2) or 0) + 10, 0)
	else
		button:SetNormalFontObject("GameFontHighlight")
		button:SetHighlightFontObject("GameFontHighlight")
		button.text:SetPoint("LEFT", (icon and DEFAULT_ICON_SIZE + ((DEFAULT_TAB_HEIGHT - DEFAULT_ICON_SIZE)/2) or 0) +10, 0) -- Change if using more than 2 levels
	end
	-- e
	...
	if disabled then
		...
		if text == "```" then
			...
			button:SetHeight(RPUI.ACDPixelMult)
	...
	if canExpand then
		...
		if not isExpanded then
			button.toggle:SetNormalTexture("Interface\\AddOns\\RasPort\\Media\\Config\\plus")
			button.toggle:SetPushedTexture("Interface\\AddOns\\RasPort\\Media\\Config\\plus") -- DF: nil throws error, "" will hide the NormalTexture on pushed (shows blank)
		else
			button.toggle:SetNormalTexture("Interface\\AddOns\\RasPort\\Media\\Config\\minus")
			button.toggle:SetPushedTexture("Interface\\AddOns\\RasPort\\Media\\Config\\minus")
		end
		-- e
		toggle:Show()
	else
...
local function Button_OnClick(frame)
	...
	PlaySoundFile("Interface\\AddOns\\RasPort\\Media\\Sound\\interface.ogg")
...
local methods = {
	...
	["CreateButton"] = function(self)
		...
		local num = AceGUI:GetNextWidgetNum("TreeGroupButton-RasPort")
		local button = CreateFrame("Button", ("AceGUI30TreeButton%d-RasPort"):format(num), self.treeframe, "OptionsListButtonTemplate")
		-- e
		button.obj = self
		...
		--button:SetWidth(150)
		button:SetHeight(DEFAULT_TAB_HEIGHT)
		--button:SetNormalTexture("Interface\\AddOns\\RasPort\\Media\\Button\\button-norm")
		...
		button.borderBottom:SetPoint("TOPRIGHT", button, "BOTTOMRIGHT", 0, RPUI.GetPixelMult())
		button.borderBottom:SetColorTexture(0, 0, 0)

		button.norm = button:CreateTexture(nil, "BACKGROUND")
		button.norm:SetTexelSnappingBias(0.0) -- works w/o ?
		button.norm:SetSnapToPixelGrid(false)
		button.norm:SetPoint("TOPLEFT", -20 - RPUI.GetPixelMult(), 0)
		button.norm:SetPoint("BOTTOMRIGHT", button.borderBottom, "TOPRIGHT", -RPUI.GetPixelMult(), 0)
		button.norm:SetTexture("Interface\\AddOns\\RasPort\\Media\\Button\\button-norm")

		button.bg = button:CreateTexture(nil, "OVERLAY")
		button.bg:SetTexelSnappingBias(0.0) -- works w/o ?
		button.bg:SetSnapToPixelGrid(false)
		button.bg:SetPoint("TOPLEFT", -20 - RPUI.GetPixelMult(), 0)
		button.bg:SetPoint("BOTTOMRIGHT", button.borderBottom, "TOPRIGHT", -RPUI.GetPixelMult(), 0)
		...
		if USE_ICON_BACKDROP then
			...
			local edgeSize = RPUI.GetPixelMult()
	...
	["RefreshTree"] = function(self,scrollToSelection,fromOnUpdate)
		...
		local maxlines = floor(	 ( height - 9 + (numDisabled * (DEFAULT_TAB_HEIGHT - RPUI.GetPixelMult())) ) / DEFAULT_TAB_HEIGHT  )
		-- e
...
local function Constructor()
	...
	local treeframe = CreateFrame("Frame", nil, frame, "InsetFrameTemplate, BackdropTemplate")
	...
	treeframe:SetBackdrop({
        bgFile = "Interface\\AddOns\\RasPort\\Media\\Background\\UI-Background-Rock-Mod.tga",
        insets = {
            left = 1,
            right = 1,
            top = 1,
	...
	treeframe:SetBackdropColor(0.25, 0.25, 0.25, 1)
	...
	local scrollbar = CreateFrame("Slider", ("AceConfigDialogTreeGroup%dScrollBar-RasPort"):format(num), treeframe, "UIPanelScrollBarTemplate")
	-- e
	...
	scrollbar.ThumbTexture:SetTexture([[Interface\AddOns\RasPort\Media\Button\button-norm]])
	...
	local border = CreateFrame("Frame", nil, frame, "InsetFrameTemplate, BackdropTemplate")
	...
	border:SetBackdropColor(0.3, 0.3, 0.3, 1)
	border:SetBackdropBorderColor(0.4, 0.4, 0.4)
	-- e
