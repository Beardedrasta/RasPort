...
local Type, Version = "CheckBox-RasPort", 31 --26 by OA, 28 backdrop 29 text right align -- v30: temp
-- e
...
local fontPath = "Fonts\\FRIZQT__.TTF"  -- Replace with your desired font path
local fontSize = 12  -- Replace with your desired font size
...
local function GetDeleteButton()
	local Button = CreateFrame("Button", "RPUI_delButton", UIParent, "BackdropTemplate")
...
--	frame.obj.checkbg:SetBackdropBorderColor(0.5, 0.5, 0.5)	 -- match range slider editbox
	frame.obj.checkbg.border:SetColorTexture(0.8, 0.6, 0)
	-- v30
...
--	frame.obj.checkbg:SetBackdropBorderColor(0.2, 0.2, 0.25) -- match range slider editbox
	frame.obj.checkbg.border:SetColorTexture(0.1, 0.1, 0.15)
	-- v30
	local arg = frame.obj.arg
	...
	if not self.disabled then
		...
		if arg and arg > 0 then
			...
			if isCtrlKey then
				...
				elseif RasPort and RasPort[1] and RasPort[1].EditSpell then
					RasPort[1].EditSpell(nil, tostring(arg))
				end
			end
		end
		--]]
...
--			self.checkbg:SetBackdropColor(0.5, 0.5, 0.5) -- s a
	...
	["SetType"] = function(self, type)
		...
		if type == "radio" then
			...
			check:SetTexture("Interface\\AddOns\\RasPort\\Media\\Button\\UI-CheckBox-Check") -- Interface\\Buttons\\UI-CheckBox-Check
			check:SetTexCoord(0, 1, 0, 1)
			check:SetBlendMode("BLEND")
			-- e
		end
	...
	["SetDescription"] = function(self, desc)
		if desc then
			if not self.desc then
				...
				local f = self.frame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
				f:SetFont(fontPath, fontSize, fontFlags)
				-- e
				f:ClearAllPoints()
...
local function Constructor()
	...
	checkbg.border:SetColorTexture(0.1, 0.1, 0.15)
...
--[[ 	checkbg.bg = checkbg:CreateTexture(nil, "BORDER")
	...
	local edgeSize = RPUI.ACDPixelMult
