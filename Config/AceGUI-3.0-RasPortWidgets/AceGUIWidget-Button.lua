...
Graphical Button.
-------------------------------------------------------------------------------]]
local Type, Version = "Button-RasPort", 26
...
local methods = {
	...
	["SetDisabled"] = function(self, disabled)
		local normalTexture = self.inner:GetNormalTexture()
		self.disabled = disabled
		if disabled then
			--[[ self.frame:Disable() ]]
			self.inner:Disable()
			--self.inner:SetBackdropColor(0.2, 0.2, 0.2)
			normalTexture:SetVertexColor(1, 1, 1)
		else
			--[[ self.frame:Enable() ]]
			self.inner:Enable()
			--self.inner:SetBackdropColor(0.725, 0.008, 0.008)
		end
	end
}
...
local function Constructor()
	...
	local frame = CreateFrame("Button", name, UIParent, "RPButtonTemplate") --"UIPanelButtonTemplate"
	frame:Hide()
	...
	local name = "AceGUI30Button-RasPort" .. AceGUI:GetNextWidgetNum(Type)
	...
	inner:SetNormalTexture("Interface\\AddOns\\RasPort\\Media\\Button\\button-norm")
	inner:SetPushedTexture("Interface\\AddOns\\RasPort\\Media\\Button\\button-pressed")
	inner:SetNormalFontObject("GameFontHighlight")
	inner:SetHighlightFontObject("GameFontHighlight")
	inner:SetDisabledFontObject("GameFontDisable")
	inner.bg = inner:CreateTexture(nil, "OVERLAY")
	inner.bg:SetTexture("Interface\\AddOns\\RasPort\\Media\\Button\\button-highlight-gold")
	if WOW_PROJECT_ID == WOW_PROJECT_CLASSIC then
		inner.bg:SetAllPoints()
	else
		inner.bg:SetTexelSnappingBias(0.0)
		inner.bg:SetSnapToPixelGrid(false)
		inner.bg:SetPoint("TOPLEFT", inner, "BOTTOMLEFT")
		inner.bg:SetPoint("BOTTOMRIGHT", inner, "TOPRIGHT")
	end
	inner.bg:Hide()
