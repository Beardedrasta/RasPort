...
local RPUI =	 LibStub("RPUI")

--[[ s r
local MAJOR, MINOR = "AceConfigDialog-3.0", 85
]]
local MAJOR, MINOR = "AceConfigDialog-3.0-RasPort", 86 -- 82 DF -- 87 backdrop
-- e
...
AceConfigDialog.tooltip = AceConfigDialog.tooltip or CreateFrame("GameTooltip", "AceConfigDialogTooltip-RasPort", UIParent, BackdropTemplateMixin and "GameTooltipTemplate, BackdropTemplate" or "GameTooltipTemplate")
AceConfigDialog.tooltip:SetPoint("BOTTOMRIGHT", -1, 3)
	    AceConfigDialog.tooltip:SetBackdrop({
	...
	AceConfigDialog.tooltip:SetBackdropColor(0.5, 0.5, 0.5, 1)
	AceConfigDialog.tooltip:SetBackdropBorderColor(0.4, 0.4, 0.4)
...
local function CleanUserData(widget, event)
	...
	if widget.type == "TreeGroup-RasPort" then
	-- e
	...
	if widget.type == "TabGroup-RasPort" then
	-- e
		widget:SetTabs(nil)
		...
		if not hidden then
			if v.type == "group" then
				if inline or pickfirstset(v.dialogInline,v.guiInline,v.inline, false) then
					...
					if name and name ~= "" then
						GroupContainer = gui:Create("InlineGroup-RasPort")
						GroupContainer:SetTitle(name or "")
					else
						GroupContainer = gui:Create("SimpleGroup-RasPort")
					end
					-- e
				...
				if v.type == "execute" then
					...
					control = CreateControl(v.dialogControl or v.control, iconControl and "Icon-RasPort" or "Button-RasPort" or "RasPortButton") -- for 'Profiles'
					-- e
					if iconControl then
						if not width then
				...
				elseif v.type == "input" then
					...
					control = CreateControl(v.dialogControl or v.control, v.multiline and "MultiLineEditBox" or "EditBox-RasPort") -- for 'Profiles'
					-- e
				...
				elseif v.type == "toggle" then
					...
					control = CreateControl(v.dialogControl or v.control, "CheckBox-RasPort") -- for 'Profiles'
					-- e
					control:SetLabel(name)
				...
				elseif v.type == "range" then
					...
					control = CreateControl(v.dialogControl or v.control, "Slider-RasPort")
					-- e
					control:SetLabel(name)
				...
				elseif v.type == "select" then
					...
					if v.style == "radio" then
						...
						control = gui:Create("InlineGroup-RasPort")
						-- e
						...
						for _, value in ipairs(sorting) do
							...
							local radio = gui:Create("CheckBox-RasPort")
							-- e
							radio:SetLabel(text)
						...
						control = CreateControl(v.dialogControl or v.control, "Dropdown-RasPort") -- for 'Profiles'
						-- e
				...
				elseif v.type == "multiselect" then
					...
					if control then
						...
						-- <spell list>
						if controlType == "InlineGroupList-RasPort" then
							control:SetLayout("Flow-Nowrap-RasPort") -- fixed width
							...
							for s = 1, #valuesort do
								...
								local text = values[value]
								local check = gui:Create("CheckBox-RasPort")
				...
				elseif v.type == "color" then
					control = CreateControl(v.dialogControl or v.control, "ColorPicker-RasPort")
					control:SetLabel(name)
				...
				elseif v.type == "description" then
					...
					if justifyH then
						...
						control:SetJustifyH("LEFT") -- default justification
					end
					-- e
...
function AceConfigDialog:FeedGroup(appName,options,container,rootframe,path, isRoot)
	...
	if (not (hasChildGroups and not inline)) or (grouptype ~= "tab" and grouptype ~= "select" and (parenttype == "tree" and not isRoot)) then
		...
		if container.type ~= "InlineGroup-RasPort" and container.type ~= "SimpleGroup-RasPort" then -- <spell list> is 'multiselect'
			scroll = gui:Create("ScrollFrame-RasPort")
			--[[ s r <spell list>
			scroll:SetLayout("flow")
			]]
			-- e
			local opt = path[#path-1]
			scroll:SetLayout(opt and opt:match("^list_%w+$") and "Flow-Nopadding-RasPort" or "flow" )
			scroll.width = "fill"
			scroll.height = "fill"
			container:SetLayout("fill")
			container:AddChild(scroll)
	...
	if hasChildGroups and not inline then
		...
		if grouptype == "tab" then
			...
			local tab = gui:Create("TabGroup-RasPort")
			-- e
		...
		elseif (parenttype ~= "tree") or isRoot then
			...
			local tree = gui:Create("TreeGroup-RasPort")
			-- e
...
function AceConfigDialog:SetDefaultSize(appName, width, height, scale)
	...
	-- s b <panel scaling>
	-- backward compatible: old direct scaling by self.Libs.ACD.OpenFramesRPSort.frame:SetScale will be
	...
	if type(scale) == "number" or scale == nil then
		...
		RPUI.globalPanelScale = status.scale
		RPUI.pixelMult = RPUI.GetPixelMult()
		RPUI.ACDPixelMult = RPUI.pixelMult / RPUI.globalPanelScale -- basis for all of our backdrop
	end
	...
	if not AceConfigDialog.isTooltipBackdropSet then
		RPUI.SetBackdrop(AceConfigDialog.tooltip)
		AceConfigDialog.tooltip:SetBackdropColor(0.3,0.3,0.3)
